...
bundle_copy<-function(gamma, dots){
  list(g_M=envelope(gamma, dots),
       g_m=squeeze(gamma, dots),
       sam_obj=sampler_obj(gamma, dots),
...
# Реализация отбора
rf<-function(starting_point, len, gamma, n, max_points = 100)
{
  inflection_point<-BS_root(gamma)
  h<-log_BS_pdf(gamma)
  h_deriv<-log_BS_pdf_deriv(gamma)
  out<-numeric(0)
  rejected<-1:n
  k<-length(rejected)
  ...
  while(k>0){
    x<-sort(unique(x))
    pack<-bundle(gamma, x)
    candidates<-rq(pack$sam_obj, k)    # Генерируем k значений
    ...
    first_rejected<-exp(pack$g_m(out[rejected])-pack$g_M(out[rejected]))<=alpha
    rejected<-rejected[first_rejected]
    # Выбираем те out, которые были приняты и которые лежат слева от inflection_point
    x<-sort(unique(c(x, out[!rejected & out<=inflection_point])))
...
gamma<-1
N<-1e7
system.time(rf_optimized(st, k, gamma, N))
