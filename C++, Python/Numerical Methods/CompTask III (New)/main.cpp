...
double fi(double x, double node, double* arg)
{
    return x - fMod(x, node, arg)/df(x);
}
...
void BiSect(double beg, double end, double h, double eps, double node, double (*f)(double, double))
{
    size_t counter = 0;
    ...
    while(x < end)
    {
        ...
        size_t m = 0;
        if(f(a, node)*f(b, node)<=0)
        {
            counter++;
            while(b - a >= 2*eps){
                m++;
                c = (a+b)/2;
                if(f(b, node)*f(c, node)<=0)
                {
                    a = c;
                } else b = c;
            }
            ...
            wcout << L"\nНевязка |f(X)-0|: " << fabs(f(c, node)) << endl;
...
void NewtonApprox(double beg, double end, double h, double eps, double node, double (*f)(double, double), double (*fi)(double, double)){
    size_t counter = 0;
    double x = beg, xNext = beg + h;
    while(x < end)
    {
...
void ModNewtonApprox(double beg, double end, double h, double eps, double node, double (*f)(double, double), double (*df)(double)){
    size_t counter = 0;
    double x = beg, xNext = beg + h;
    while(x < end)
    {
...
void SecantApprox(double beg, double end, double h, double eps, double node, double (*f)(double, double), double (*secf)(double, double, double)){
     size_t counter = 0;
     double x = beg, xNext = beg + h;
     while(x < end){
        double a = x, b = xNext;
        double c = secf(b, a, node), delta, fappr=secf(b, a, node);
        size_t m = 0;
        if(f(a, node)*f(b, node)<=0){
            counter++;
            while(fabs(c - b) >= eps){
...
void procedureTwo(){
    ...
    while(!(flag-1))
    {
        ...
        wcout << L"(Погрещность) eps = ";cin >> eps;
        while(eps <= 0)
        {
        ...
        SecantApprox(a, b, h, eps, x, fMod, secf);
...
// void procedureOne(double &a, double &b, double &x, double &m, double &n, double &eps);

// void procedureTwo(double &a, double &b, double &x, double &m, double &n, double &eps);

void procedureOne();

void procedureTwo();

double fMod(double x, double node, double* arg);
