...
void ChebyschevTriangleMethod::Calculate(const RESOLVER& resolver)
{
	...
	gamma_1 = delta_small / (2 + 2 * sqrt(nu));
	gamma_2 = delta_small / (4 * sqrt(nu));
	// Параметр тау
	resU = resolver.GetU();
	for (sizetype i = 1; i <= iter_num; ++i) {
		tau = 2. / 
			(gamma_1 + gamma_2 + (gamma_2 - gamma_1) * cos(M_PI_2 * (2 * i - 1) / iter_num));
		IterateOnce(resolver);
	}
}

void ChebyschevTriangleMethod::ChangeIterNum(sizetype _iter_num)
{
	iter_num = _iter_num;
}

void ChebyschevTriangleMethod::PrintName()
{
	cout << endl << "Chebyschev Triangle Method. Variant 4" << endl;
}

void ChebyschevTriangleMethod::ChangeOmega(double _omega)
{
	omega = _omega;
}

void ChebyschevTriangleMethod::ChangeTau(double _tau)
{
	tau = _tau;
}

void ChebyschevTriangleMethod::IterateOnce(const RESOLVER& resolver)
{
