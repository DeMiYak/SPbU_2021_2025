...
matrixLine *matrixLine_resize(matrixLine *m, size_t i, size_t j)
{
    ...
    /*
    double *cpy = malloc(m->vert*m->hor*sizeof(double));
    double *temp = m->mat, *tempEnd = m->mat + m->vert*m->hor;
    while(temp < tempEnd)
    {
        *(cpy++) = *(temp++);
    }

    double *cpyVert = cpy, *cpyVertEnd = cpy + m->vert;
    m->mat = realloc(m->mat, i*j*sizeof(double));
    matrixLine_print(m);
    printf("\n1");
    double *ptrVert = m->mat, *endVert = m->mat + i;
    int check = m->hor > j ? 1 : 0;
    for(; ptrVert < endVert || cpyVert < cpyVertEnd; ++ptrVert, ++cpyVert)
    {
        // Рассмотрим несколько ситуаций:
        // 1. Кол-во эл-тов в строке меньше, чем в исходной - записываем до j циклом
        // 2. Кол-во эл-тов в строке больше, чем в исходной - записываем до m->hor + нули для k > m->hor
        // 3. Кол-во эл-тов в столбце больше, чем в исходном - игнорируем
        // 4. Кол-во эл-тов в столбце меньше, чем в исходном - смотрим на i
        // 3 + 4 -> цикл до i
        double *ptrHor = m->mat + (ptrVert - m->mat)*j, *endHor = m->mat + (ptrVert - m->mat)*(j+1);
        double *cpyHor = cpy + (cpyVert - cpy)*m->hor, *cpyEndHor = cpy + (cpyVert - cpy)*(m->hor+1);
        if(check)
        {
            while(ptrHor < endHor)
            {
